...
namespace Vispl.Trainee.CricInfo.DL.ITF
{
    public interface IMatchSqlDL
    {
        ...
        List<Dictionary<string, object>> GetBattingDataForTeamAndMatch(int teamId, int matchId);
        List<Dictionary<string, object>> GetFallOfWicket(int teamId, int matchId);
    }
}
