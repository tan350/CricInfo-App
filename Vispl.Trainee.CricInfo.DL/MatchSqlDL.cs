...
JOIN 
    Teams t2 ON m.Team2 = t2.TeamID
        ...
        public void SaveWicketData(WicketUpdateVO wicket)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
        ...
        public List<WicketUpdateVO> GetWicketsByMatchId(int matchId)
        {
            List<WicketUpdateVO> wickets = new List<WicketUpdateVO>();
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            WicketUpdateVO wicket = new WicketUpdateVO
                            {
                                MatchId = reader.GetInt32(reader.GetOrdinal("matchId")),
        ...
        public void UpdateFallOfWicket(WicketUpdateVO model)
        {
        ...
        public List<Dictionary<string, object>> GetBattingDataForTeamAndMatch(int teamId, int matchId)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                string query = @"
                    ...
                    FROM
                        ...
                        INNER JOIN Teams T ON (M.Team1 = T.TeamID OR M.Team2 = T.TeamID)
                    WHERE
                        P.TeamId = @TeamID
                        AND T.TeamID = @TeamID
                ...
                while (reader.Read())
                {
                    ...
                    // Map each column to the dictionary
                    row["PlayerName"] = reader["PlayerName"];
                    row["MatchID"] = reader["MatchID"];
                    row["Runs"] = reader["Runs"];
                    row["Balls"] = reader["Balls"];
                    row["Fours"] = reader["Fours"];
                    row["Sixes"] = reader["Sixes"];
                    row["StrikeRate"] = reader["StrikeRate"];
        ...
        public List<Dictionary<string, object>> GetFallOfWicket(int teamId, int matchId)
        {
            List<Dictionary<string, object>> fallOfWicketData = new List<Dictionary<string, object>>();

            string query = @"
        SELECT
    P.Name AS BatsmanName,
    SUM(Bat.Runs) AS TeamRuns, -- Sum of runs scored by the team till the current ball
    COUNT(FOW2.wicketId) AS TeamWickets, -- Count of wickets lost by the team till the current ball
    CONCAT((SUM(Bat.Balls) / 6), '.', (SUM(Bat.Balls) % 6)) AS TeamOvers -- Calculation of overs in the format overs.balls
FROM
    fallOfWicket FOW
    INNER JOIN Players P ON FOW.batsmanId = P.PlayerId
    INNER JOIN Players B ON FOW.bowlerId = B.PlayerId
    LEFT JOIN Players F ON FOW.fielderId = F.PlayerId
    INNER JOIN wicketType WT ON FOW.wicketTypeId = WT.wicketTypeId
    INNER JOIN Matches M ON FOW.matchId = M.MatchID
    INNER JOIN Teams T ON P.TeamId = T.TeamID
    LEFT JOIN Batting Bat ON Bat.MatchId = FOW.matchId AND Bat.PlayerId = P.PlayerId AND Bat.Balls <= 
    (SELECT SUM(Balls) FROM Batting WHERE MatchId = FOW.matchId AND PlayerId = P.PlayerId) -- Running total of balls till the current fall of wicket
    LEFT JOIN fallOfWicket FOW2 ON FOW2.matchId = FOW.matchId AND FOW2.wicketId <= FOW.wicketId AND FOW2.batsmanId IN (SELECT PlayerId FROM Players WHERE TeamId = T.TeamID)
WHERE
    P.TeamId = @TeamId AND FOW.matchId = @MatchId
GROUP BY
    FOW.wicketId,
    FOW.matchId,
    P.Name
    ...
    FOW.wicketId;";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var wicket = new Dictionary<string, object>
                    {
                        {"BatsmanName", reader["BatsmanName"]},
                        {"TeamRuns", reader["TeamRuns"]},
                        {"TeamWickets", reader["TeamWickets"]},
                        {"TeamOvers", reader["TeamOvers"]}
                    };
                        fallOfWicketData.Add(wicket);
                    }
                }
            }

            return fallOfWicketData;
        }
